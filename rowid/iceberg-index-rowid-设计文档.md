...
**已完成**：SDK pipeline 已实现原生 `_pos` (UInt64) 和 `_row_id` (Int64) 列读取。在 delete filter 后、transformer 前生成，保证只计数存活行。详见 `iceberg-rust/crates/iceberg/src/arrow/reader/pipeline.rs`。
...
**已完成**：索引层移除手动计数器，使用 SDK 原生 `_pos` 列。DELETE 后的索引清理（I5 `RowIdMapping::remove_row_ids()` + `IndexPlugin::prune()`）也已实现。
...
### 5.8 阶段三：插件 RowID 适配与维护 (P2-P7) ✅ 已完成

> **状态**: 全部完成。插件不再自建 file_index_map 或 file_path 缓存，直接存储 SDK RowID。

| 编号 | 内容 | 层 | 说明 |
|------|------|----|------|
| P2 | BTree RowID 适配 | 插件 | 数据页 Int64 SDK RowID，移除 pack/unpack_row_id |
| P3 | IVF RowID 适配 | 插件 | 二进制格式改为 row_ids 直存，移除字符串表 |
| P4 | IVF-PQ RowID 适配 | 插件 | 同 IVF，分区存 PQ codes + row_ids |
| P5 | BTree prune() | 插件 | page 级 RowID 过滤 + lookup 重建 |
| P6 | IVF prune() | 插件 | 分区级 RowID 过滤 + 重写 |
| P7 | IVF-PQ prune() | 插件 | 分区级 RowID 过滤 + 重写 |

**核心改造**:
- `_file` / `_row_id` 是 SDK 原生列，不通过 JSON 参数配置（硬编码 const）
- 查询时返回 `ScoredRowId { row_id: u64, score: f32 }`（向量）或 `u64` row_id（标量）
- table 层 `IndexSearchCoordinator::resolve_scored_rows()` / `ScalarSearchCoordinator::resolve_row_ids()` 统一通过 RowIdMapping 解析
- `prune()` trait 签名接受 `PluginContext` + `IndexSegmentMetadata`，替代独立函数
...
| **S2+M4 已完成** | I6 Remap 可用方案 B（M4 dual-read） | ✅ 物理 `_row_id` 列优先读取，fallback 动态计算 |
| **增量扫描已实现** (M5) | I4 INSERT 增量构建 | ✅ `appends_after(from_snapshot_id)` |
...
| Rust SDK #2746      | https://github.com/apache/iceberg-rust/pull/2746                                | `_pos` 列读取             | ✅ 已实现（fork） |
| Rust SDK #2153      | https://github.com/apache/iceberg-rust/pull/2153                                | 增量扫描 `appends_after()` | ✅ 已实现（fork） |
