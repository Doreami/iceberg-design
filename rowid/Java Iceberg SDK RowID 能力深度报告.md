...
| 列类型           | 列名                                   | 字段 ID      | 说明                        |
| ------------- | ------------------------------------ | ---------- | ------------------------- |
| **隐藏列（物理存储）** | `_row_id`                            | 2147483540 | 物理存储在 Parquet 文件中         |
...
| 场景                  | 读取方式                                | 说明          |
| ------------------- | ----------------------------------- | ----------- |
| **新写入的数据（有物理列）**    | 直接读取 `_row_id`                      | 最快，O(1) 读取  |
| **历史数据（无物理列）**      | 动态计算 `first_row_id + FILE_POSITION` | 兼容性保障，有计算开销 |
| **Compaction 后的数据** | 直接读取 `_row_id`                      | 新文件保留了物理列   |
...
- **隐藏列（物理存储）**：`_row_id`、`_file_last_updated_sequence_number`
